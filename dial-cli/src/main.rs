...
async fn run_api_client(args: Args) -> Result<()> {
    ...
    if !args.repl {
        ...
        if args.stream {
            ...
            let (_content, ttft, total, tps, decode_tps, dist_overhead, remote_compute, remote_requests) =
    ...
    if !args.prompt.is_empty() {
        ...
        if args.stream {
            let resp = http
                ...
                .error_for_status()?;
            print_model_header(&args.model);
            let (content, ttft, total, tps, decode_tps, dist_overhead, remote_compute, remote_requests) =
                consume_sse_stream(resp, true).await?;
