...
impl Qwen3Vl {
    ...
    fn encode_video_bytes(
        ...
        log::info!(
            "video sample plan: source={}x{} fps={:.3} source_frames={} processed_frames={} mode={} target_fps={:.3}",
            probe.width,
            probe.height,
            probe.fps,
            total_frames,
            sample_indices.len(),
            if self.ctx.args.video_no_sample { "all" } else { "official" },
            self.ctx.args.video_fps
        );
        ...
        log::info!(
            "starting video decode: ffmpeg={} selected_frames={} output={}x{}",
            command_from_env(
                "DIAL_FFMPEG_BIN",
                "ffmpeg",
                "/opt/sophon/sophon-ffmpeg-latest/bin/ffmpeg"
        ...
        let total_patch_groups = patch_groups.len();
        let total_batches = (total_patch_groups + batch_size - 1) / batch_size;
        for (batch_no, batch) in patch_groups.chunks(batch_size).enumerate() {
            log::info!(
                "video vision batch {}/{}: temporal_patches={} sampled_frames={}/{}",
                batch_no + 1,
                total_batches,
                batch.len(),
                ((batch_no + 1) * batch_size * temporal).min(expected_frames),
                expected_frames
            );
            let started = prompt_profile.then(Instant::now);
            ...
            for (patch_no, group) in batch.iter().enumerate() {
                let patch_embeds = batch_embeds.narrow(0, patch_no, 1)?.squeeze(0)?;
                ...
                for output in &batch_deepstack {
                    patch_deepstack.push(output.narrow(0, patch_no, 1)?.squeeze(0)?);
                }
                deepstack_embeds.push(patch_deepstack);
            }
            log::info!(
                "video vision batch {}/{} complete",
                batch_no + 1,
                total_batches
            );
        }
