...
impl<G: Generator + 'static> Worker<G> {
    /// Actual listen endpoint, including OS-assigned ports for local tests.
    ...
    pub async fn new(ctx: Context) -> Result<Self> {
        ...
        if let Some((linears, bytes)) =
            crate::models::qwen3_8::quant_linear_summary().filter(|(linears, _)| *linears > 0)
        {
            log::info!(
                "Qwen3.8 CUDA QuantLinear worker summary: linears={} resident_weights={:.1} MiB",
                linears,
        ...
        if let Some(engine) = &ctx.config.qwen3_8_ggml {
            ...
            log::info!("Qwen3.8 GGML worker: assigned_layers={} resident_tensors={tensors} quantized_weight_storage={:.1} MiB (no embedding/output head)", layer_names.len(), bytes as f64 / 1048576.0);
        }
    ...
    async fn handle_master_client(
            ...
            Self::read_message_timed(&mut socket).await
        {
            ...
            // GGML can execute a consecutive shard as one graph. Check every
            // requested layer before entering the fast path, including layers
            // after the first, so an invalid batch cannot mutate partial state.
            let fused = if ops
                .iter()
                .all(|(name, _, _)| context.blocks.contains_key(name))
            {
                ops.first().and_then(|(name, _, _)| {
                    context
                        .blocks
                        .get(name)?
            ...
            let fused_used = fused.is_some();
            let remaining_ops = match fused {
                Some(Ok(output)) => {
                    x = output;
                    Vec::new()
                }
                Some(Err(error)) => {
                    remote_error = Some(anyhow!(
                        "[{}] fused forward failed for {}: {error}",
                        &client,
                        ops_summary
                    ));
                    Vec::new()
            ...
            for (layer_name, index_pos, block_idx) in remaining_ops {
                // 根据模型层名获取模型层
                if let Some(block) = context.blocks.get(&layer_name) {
                    ...
                    remote_error = Some(anyhow!("could not find layer {}", &layer_name));
                    break;
                }
            }
            ...
            if let Some(error) = remote_error {
                ...
                log::error!("{}", message);
                Self::write_message_timed(&mut socket, Message::Error { message })
                    .await
                    .map_err(|write_error| {
                        anyhow!("[{}] could not send worker error: {}", &client, write_error)
                    })?;
                continue;
            }
            ...
            .await
            {
                Ok((elaps_write, written)) => {
                    ...
                    if Self::transfer_trace_enabled() {
                        log::info!(
                            "[transfer][worker {} msg={}] {} fused={} read={}B/{:.3}ms decode={:.3}ms execute={:.3}ms encode={:.3}ms compute={:.3}ms write={}B/{:.3}ms total={:.3}ms",
                            &client,
                            msg_idx,
                            ops_summary,
                            fused_used,
